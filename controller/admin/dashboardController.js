...
const downloadExcelReport = async (req, res) => {
  try {
    ...
    const totalItems = orders.reduce((sum, order) => {
      const itemCount = order.orderItems ? order.orderItems.reduce((itemSum, item) => itemSum + (item.quantity || 1), 0) : 0
...
const downloadPdfReport = async (req, res) => {
  try {
    ...
    const columnWidths = [tableWidth * 0.2, tableWidth * 0.15, tableWidth * 0.15, tableWidth * 0.15, tableWidth * 0.15, tableWidth * 0.2]
    const drawTableHeader = (y) => {
      ...
      doc.text("PAYMENT", startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + 10, headerY, { width: columnWidths[4] - 10, align: 'center' })
      doc.text("DATE", startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + columnWidths[4] + 10, headerY, { width: columnWidths[5] - 10, align: 'center' })
      return y + 25
    }
    ...
    orders.forEach((order, index) => {
      ...
      doc.fillColor('#4B5563')
      doc.text(paymentMethod, startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + 10, rowTextY, { width: columnWidths[4] - 10, align: 'center' })
      doc.text(new Date(order.createdOn).toLocaleDateString(), startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + columnWidths[4] + 10, rowTextY, { width: columnWidths[5] - 10, align: 'center' })
      rowY += 30
    })
    
    const boxX = 50, boxY = rowY + 20, boxWidth = doc.page.width - 100, boxHeight = 140
...
const getOrdersReport = async (req, res) => {
  try {
      ...
      res.render("salesReport", {
          ...
          totalDiscount,
          totalFinalAmount,
          paymentMethods: orders.map(order => order.paymentMethod || "N/A") 
      });
  } catch (error) {
